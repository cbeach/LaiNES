...
template<bool wr> inline u8 access(u16 addr, u8 v = 0)
{
    ...
    switch (addr)
    {
        ...
        case            0x4017:  if (wr) return APU::access<wr>(elapsed(), addr, v);
                                 else return Joypad::read_state(1, input_state.nes_console_state().player2_input());                  // Joypad 1.

        case            0x4014:  if (wr) dma_oam(v); break;                          // OAM DMA.
        case            0x4016:  if (wr) { Joypad::write_strobe(v & 1, input_state.nes_console_state().player1_input()); break; }     // Joypad strobe.
                                 else return Joypad::read_state(0, input_state.nes_console_state().player1_input());                  // Joypad 0.
